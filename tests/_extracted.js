...
function crossTrackError(lat, lng, start, end) {
      ...
      const atAng = Math.acos(Math.cos(d13) / Math.cos(xtAng));
      return { xte: xtAng * R, along: (isNaN(atAng) ? 0 : atAng * R) };
    }
...
function fmtCoord(value, type) {
      ...
      return `${deg}°${min.toFixed(1).padStart(4, '0')}'${hemi}`;
    }
...
function getCoastline() {
      if (!_coastline) {
        _coastline = lighthouses
          .filter(lh => !OCEANIC_LIGHTHOUSES.includes(lh.name))
...
module.exports = { lighthouses, DEFAULT_EYE_HEIGHT_M, setTrip: t => { tripData = t; }, calculateDistance, calculateBearing, eyeHeight, calculateVisibility, effectiveRange, findNearestLighthouse, crossTrackError, elapsedFuel, fmtDuration, fmtCoord, offsetLatLng, getCoastline, distanceFromCoast };